...
contract ZkMinimalAccountTest is Test {
    ...
    // testを助ける役目、vmはTest.solをimport、vmなどtest目的のものはZkMinimalAccount.solなどの本番環境に置かない
    function _signTransaction(Transaction memory transaction) internal view returns (Transaction memory) { // address account → none; ANVIL_DEFAULT_KEYを使用しているから不要
        ...
        (v, r, s) = vm.sign(ANVIL_DEFAULT_KEY, unsignedTransactionHash); // digest → unsignedTransactionHash
        Transaction memory signedTransaction = transaction;
        signedTransaction.signature = abi.encodePacked(r, s, v); // transaction構造的のsignatureフィールドに格納
